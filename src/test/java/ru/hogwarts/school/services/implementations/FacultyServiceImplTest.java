...
public class FacultyServiceImplTest {
    ...
    void createFaculty_facultyIsAlreadyInTable_thrownException() {
        when(testRepository.findByNameAndColor(faculty.getName(), faculty.getColor()).isPresent())
                .thenThrow(new FacultyException("FACULTY IS ALREADY IN TABLE"));
    ...
    @Test
    void readFaculty_facultyIsNotInTable_returnFaculty() {
        when(testRepository.findById(faculty.getId()).isEmpty())
                .thenThrow(new FacultyException("FACULTY NOT FOUND"));
        FacultyException ex = assertThrows(FacultyException.class,
    ...
    @Test
    void updateFaculty_facultyIsNotInTable_facultyUpdatedAndReturned() {
        when(testRepository.findById(faculty.getId()).isEmpty())
                .thenThrow(new FacultyException("FACULTY NOT FOUND"));
        FacultyException ex = assertThrows(FacultyException.class,
    ...
    @Test
    void deleteFaculty_facultyIsNotInTable_facultyDeletedAndReturned() {
        when(testRepository.findById(faculty.getId()).isEmpty())
                .thenThrow(new FacultyException("FACULTY NOT FOUND"));
        FacultyException ex = assertThrows(FacultyException.class,
